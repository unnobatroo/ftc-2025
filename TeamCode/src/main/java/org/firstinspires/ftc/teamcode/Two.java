...
@TeleOp(name="Two", group="Linear OpMode")
public class Two extends OpMode {
    ...
    private static final double Kp = 0.0012; // Уменьшенный коэффициент пропорциональности
    private static final double DEAD_ZONE = 1; // Мёртвая зона для heading
    ...
    public void loop() {
        ...
        if (pos.getY(DistanceUnit.MM) > -500 ) {
            backright.setPower(-0.3);
            frontright.setPower(0.3);
            backleft.setPower(0.3);
            frontleft.setPower(-0.3);
        } else if (pos.getY(DistanceUnit.MM) < -550) {
            backright.setPower(0.3);
            frontright.setPower(-0.3);
            backleft.setPower(-0.3);
            frontleft.setPower(0.3);
